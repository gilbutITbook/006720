...
#define ROT_R				180.0f						// 회전반경 
#define ANGLE_VEL			( 2.0f * PI / 120.0f )			// 각속도 


float				x, y;							// 표시위치 
float				rx, ry;							// 회전중심으로부터의 상대위치 
float				vx, vy;							// 속도 
...
int InitCharacter( void )							// 처음에 한 번만 호출된다 
{
	...
	vx = 0.0f;										// 초속 
	vy = ROT_R * ANGLE_VEL;
	x = rx + ( VIEW_WIDTH  - CHAR_WIDTH  ) / 2.0f;	// 표시위치 
...
int MoveCharacter( void )							// 매 프레임 호출된다
{
	rx += vx;										// 위치에 속도를 가한다 
	ry += vy;
	vx += -ANGLE_VEL * ANGLE_VEL * rx;				// 속도에 가속도를 가한다 
