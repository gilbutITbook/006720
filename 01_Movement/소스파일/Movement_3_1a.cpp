...
#define PLAYER_VEL			6.0f					// 플레이어 속도 
...
{
	x = ( float )( VIEW_WIDTH  - CHAR_WIDTH  ) / 2.0f;	// 초기위치 
	y = ( float )( VIEW_HEIGHT - CHAR_HEIGHT ) / 2.0f;
	fAngle = PI / 6.0f;
	vx = PLAYER_VEL * cosf( fAngle );			// 초속 설정 
...
int MoveCharacter( void )						// 매 프레임 호출된다 
{
	...
	// 화면 밖으로 밀려나면 초기 위치로 돌아온다 
	if ( ( x < -CHAR_WIDTH )  || ( x > VIEW_WIDTH  ) ||
		 ( y < -CHAR_HEIGHT ) || ( y > VIEW_HEIGHT ) )
	{
		x = ( float )( VIEW_WIDTH  - CHAR_WIDTH  ) / 2.0f;
		y = ( float )( VIEW_HEIGHT - CHAR_HEIGHT ) / 2.0f;
		fAngle += 2.0f * PI / 10.0f;						// 각도를 증가한다 
		if ( fAngle > ( 2.0f * PI ) ) fAngle -= 2.0f * PI;	// 한 바퀴 돌면 각도를 되돌린다 
