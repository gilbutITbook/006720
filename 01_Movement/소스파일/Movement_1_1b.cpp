...
int InitCharacter( void )						// 처음 한 번만 호출된다
{
	x = 0;										// 캐릭터의 초기 위치
	v = 10;										// 캐릭터의 x방향 속도 ①
...
int MoveCharacter( void )						// 매 프레임 호출된다 
{
	...
	if ( x > VIEW_WIDTH - CHAR_WIDTH ) {		// 오른쪽 끝 ②
		v = -v;									// 되돌아 온다 ③
		x = VIEW_WIDTH - CHAR_WIDTH;			// 정확히 벽 위치로 ④
	}
	if ( x < 0 ) {								// 왼쪽 끝
		v = -v;									// 되돌아 온다 
		x = 0;									// 정확히 벽 위치로 
...
int Cleanup( void )
{
    ...
    SAFE_RELEASE( g_pImmediateContext );					// 디바이스 컨텍스츠 
...
{
	LARGE_INTEGER			nNowTime, nLastTime;		// 현재와  바로 직전 시각 
