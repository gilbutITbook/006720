...
#define PIC_WIDTH_UP		960					// 그림 위 너비 
#define PIC_WIDTH_DOWN		1920				// 그림 아래 너비 
#define CAMERA_VEL			10.0f				// 카메라 빠르기 
...
DRAWPOINT		v2Points[VIEW_HEIGHT];			// 그리는 라인의 위치  


int InitBack( void )							// 처음에 한 버난 호출된다 
{
...
{
	int						i;
	float					fLineWidth;			// 현재 라인의 그림 너비 
	float					fLineBase;			// 라인의 기본 위치 
...
#include <stdio.h>
#include <windows.h>
#include <tchar.h>								// Unicode 멀티 바이트 문자 체계
    ...
    if( SUCCEEDED( InitD3D() ) && SUCCEEDED( MakeShaders() ) )
    {
        ...
        if( SUCCEEDED( InitDrawModes() ) )
        {
			if ( SUCCEEDED( InitGeometry() ) ) {					// 지오메트리 작성 
				...
				QueryPerformanceCounter( &nLastTime );				// 1 프레임 전 시각 초기화 
