...
int MoveBack( void )							// 매 프레임 호출된다 
{
	...
	for ( i = 0; i < VIEW_HEIGHT; i++ ) {
		v2Points[i].y = ( float )i;				// 그리는 라인의 y좌표 
		v2Points[i].x = ( float )i;				// 그리는 라인의 x좌표 
	}
...
{
	if ( g_nVertexNum > ( MAX_BUFFER_VERTEX - 6 ) ) return -1;	// 정점이 버퍼로부터 넘칠 때는 그리지 않고
...
{
	LARGE_INTEGER			nNowTime, nLastTime;		// 현재와 직전의 시간 
