...
int MoveChara( void )							// 매 프레임 호출된다 
{
	...
	if ( fCamera_x < fChara_sx - SCROLL_DIF ) {		// 카메라가 왼쪽으로 접근하는지 체크 
		fCamera_x = fChara_sx - SCROLL_DIF;
	}
	if ( fCamera_x > fChara_sx + SCROLL_DIF ) {		// 카메라가 오른쪽으로 접근하는지 체크 
		fCamera_x = fChara_sx + SCROLL_DIF;
	}
...
HRESULT InitGeometry( void )
{
	...
	hr = LoadTexture( _T( "8_그림자있음.dds" ), &g_tChara );
    if ( FAILED( hr ) ) {
 		MessageBox( NULL, _T( "Can't open 8_그림자있음.dds" ), _T( "Error" ), MB_OK );
       return hr;
    }
