...
int InitBack( void )							// 최초에 한 번만 호출된다 
{
	fCamera_x = VIEW_WIDTH / 2.0f;				// 카메라 초기위치①
	fBack_x = VIEW_WIDTH / 2.0f - fCamera_x;	// 배경 초기위치②
...
int MoveBack( void )							// 매 프레임 호출된다
{
	...
	fBack_x = VIEW_WIDTH / 2.0f - fCamera_x;	// 배경위치⑤
...
#define MAX_BUFFER_VERTEX				1000	// 최대버퍼 정점 수 
...
{
	if ( g_nVertexNum > ( MAX_BUFFER_VERTEX - 6 ) ) return -1;	// 정점 버퍼에서 넘칠 때는 그리지 않고 
...
{
	LARGE_INTEGER			nNowTime, nLastTime;		// 현재와 바로 전 시각 
