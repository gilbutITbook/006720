...
#define GRAD_WIDTH			100					// 그라데이선 너비 
#define DRAW_R				( VIEW_WIDTH + VIEW_HEIGHT + GRAD_WIDTH * 2 )	// 그리기 반경 
...
int Draw2DPolygon( float x1, float y1, float u1, float v1,
				   ...
				   TEX_PICTURE *pTexPic );		// 2D 폴리곤 그리기 
int Draw2DPolygonWithColor( float x1, float y1, float u1, float v1, int nColor1,
							...
							TEX_PICTURE *pTexPic );	// 2D 폴리곤 그리기（색）
...
int InitChangingPictures( void )						// 처음에 한 번만 호출된다. 
{
	fAngle1 = 0.0f;									// 각도 초깃값 
	fAngle_v = 2.0f * PI / 300.0f;						// 각속도 초깃값 
...
{
	float				xb1, yb1, xt1, yt1;			// 경계선분1의 점
	float				xb2, yb2, xt2, yt2;			// 경계선분2의 점 
	float				dlx, dly;					// 왼쪽아래 점 
	float				urx, ury;					// 오른쪽 위 점 
	float				vfx, vfy;					// 포워드 벡터 
	float				vsx, vsy;					// 사이드 벡터 

	fAngle1 += fAngle_v;								// 경계선을 움직인다 
	if ( fAngle1 < 0.0f ) {							// 최소각도 
		fAngle1 = 0.0f;
		fAngle_v = -fAngle_v;
	}
	if ( fAngle1 > PI / 2.0f ) {						// 최대각도 
		fAngle1 = PI / 2.0f;
		fAngle_v = -fAngle_v;
	}
...
{
	LARGE_INTEGER			nNowTime, nLastTime;		// 현재와 한 프레임 앞의 시각 
