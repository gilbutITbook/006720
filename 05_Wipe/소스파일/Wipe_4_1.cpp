...
#define DIVIDE_NUM			50					// 분할 수
#define MIN_R				10.0f				// 최소 반지름
...
ID3D11DeviceContext		*g_pImmediateContext;	// 디바이스 컨텍스처 
ID3D11BlendState		*g_pbsAlphaBlend;


int FlushDrawingPictures( void );				// 그림의 그리기 대기행렬 플러시
...
{
	fBoundary_r1 = MIN_R;								// r1의 초깃값 
	fBoundary_r2 = sqrtf( VIEW_WIDTH * VIEW_WIDTH + VIEW_HEIGHT * VIEW_HEIGHT ) / 2.0f;	// r2의 초깃값 
	fBoundary_v = 5.0f;										// 경계의 속도
...
int DrawChangingPictures( void )						// 매 프레임 호출된다 
{
	...
	float				fAngleDelta;
	float				xt[4], yt[4];					// 위쪽 직선상의 점 

	fBoundary_r1 += fBoundary_v;						// 경계선을 움직인다 
	if ( fBoundary_r1 < MIN_R ) {						// 최소원 
		fBoundary_r1 = MIN_R;
		fBoundary_v = -fBoundary_v;
	}
	if ( fBoundary_r1 > fBoundary_r2 ) {				// 최대원 
