...
{
	fBoundary_x = 0.0f;									// 경계의 초기 위치
	fBoundary_vx = 10.0f;								// 경계의 x 방향 속도
...
{
	fBoundary_x += fBoundary_vx;						// 경계선을 움직인다 
	if ( fBoundary_x < 0.0f ) {							// 화면 왼쪽 끝
		fBoundary_x = 0.0f;
		fBoundary_vx = -fBoundary_vx;
	}
	if ( fBoundary_x > VIEW_WIDTH ) {					// 화면 오른쪽 끝 
