...
#define GRAD_WIDTH			200					// 그라디에이션 너비 
...
int FlushDrawingPictures( void );				// 그림 그리기 대기행렬 플러시 
...
{
	fBoundary_t = ( float )-VIEW_WIDTH / 2.0f - GRAD_WIDTH;	// t의 초깃값 
	fBoundary_v = 7.0f;										// 경계의 속도 
...
{
	int					i;
	float				xt[4], yt[4];					//  위쪽 직선 상의 점
	float				xb[4], yb[4];					// 아래쪽 직선 상의 점 

	fBoundary_t += fBoundary_v;							// 경계선을 움직인다 
	if ( fBoundary_t < -VIEW_WIDTH / 2.0f - GRAD_WIDTH ) {	// 화면 왼쪽 끝 
		fBoundary_t = -VIEW_WIDTH / 2.0f - GRAD_WIDTH;
		fBoundary_v = -fBoundary_v;
	}
	if ( fBoundary_t > VIEW_HEIGHT / 2.0f ) {				// 화면 오른쪽 끝
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    if( SUCCEEDED( InitD3D() ) && SUCCEEDED( MakeShaders() ) )
    {
        ...
        {
			if ( SUCCEEDED( InitGeometry() ) ) {					// 지오메트리작성 
