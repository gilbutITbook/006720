...
#define CIRCLE_VEL			5.0f				// 원형의 빠르기
...
F_RECT_CIRCLE		rcRectCircleA;		// 사각형 + 원A
F_RECT_CIRCLE		rcRectCircleB;		// 사각형 + 원B


int InitShapes( void )						// 처음 한 번만 호출된다
{
	// 선분A
...
//	rcRectCircleB.vx = 100.0f;  rcRectCircleB.vy = -50.0f;		// 선분A와 B가 평행인 경우를 체크 
...
int CheckHit( F_RECT_CIRCLE *prcRectCircle1, F_RECT_CIRCLE *prcRectCircle2 )		// 충돌 검사 
{
	int				nResult = false;
	static int		nCount = 0;
...
int MoveRect( void )						// 키 입력으로 사각형A를 이동
{
//	float			fVelocity;
...
ID3D11DeviceContext		*g_pImmediateContext;	// 디바이스 컨텍스츠 
...
{
	LARGE_INTEGER			nNowTime, nLastTime;		// 현재와 하나 직전의 시각
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "CheckHit_3_2(Secret:선분과 선분, 바탕 소스)" ),
