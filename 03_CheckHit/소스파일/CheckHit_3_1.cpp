...
int InitShapes( void )					// 처음에 한 번만 호출된다 
{
...
int CheckHit( F_CIRCLE *pcrCircle, F_RECT_CIRCLE *prcRectCircle )		// 충돌 체크 
{
	int				nResult = false;
	float			dx, dy;							// 위치의 차이 
	float			t;
...
int MoveRect( void )						// 키 입력으로 사격형A를 이동 
{
//	float			fVelocity;
...
// 글로벌 변수 
UINT  g_nClientWidth;							// 그리기 영역의 너비 
UINT  g_nClientHeight;							// 그리기 영역의 높이 
