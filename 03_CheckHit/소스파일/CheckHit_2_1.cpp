...
int InitCircle( void )						// 처음 한 번만 호출된다 
{
...
int CheckHit( F_CIRCLE *pcrCircle1, F_CIRCLE *pcrCircle2 )		// 충돌 체크 
{
	int				nResult = false;
	float			dx, dy;							// 위치의 차이 
	float			ar;								// 2 반지름을 더한 것 
	float			fDistSqr;
...
int MoveRect( void )						// 키 입력으로 원형A를 이동 
{
//	float			fVelocity;
...
ID3D11DeviceContext		*g_pImmediateContext;	// 디비아스 컨텍스트 
